      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-6 sm:p-7 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-5">
        <div className="space-y-1.5">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
            <span>{t('Company')}: <strong className="text-slate-800 font-bold">{session.companyName}</strong></span>
            <span>•</span>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-black text-slate-900 flex items-center space-x-2">
              ...
              <span>{t('Available Tenders for Bidding')}</span>
            </h2>
            <p className="text-xs text-slate-500 mt-0.5">
              {t('Published procurement RFPs open for technical qualification and document submission.')}
            </p>
          </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 mt-1 flex items-center space-x-1 truncate">
                        ...
                        <span className="truncate">{t(tItem.department)}</span>
                      </p>
                    </div>
                    ...
                    <div className="flex items-center space-x-1.5 text-xs text-slate-600 bg-slate-50/80 px-2.5 py-1.5 rounded-xl">
                      ...
                      <span className="font-semibold">{mandatoryCount} {t('Mandatory Files Required')}</span>
                    </div>
                  </div>
      ...
      <div className="space-y-4 pt-2" id="submissions">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500 font-medium">
            {t('Active Tender Evaluation Standing')}
          </span>
        </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="space-y-1 max-w-3xl">
              ...
              <p className="text-xs text-slate-500 flex items-center space-x-1">
                ...
                <span>{t(MOCK_TENDER.department)}</span>
              </p>
            </div>
            ...
            <div className="flex items-center space-x-3 shrink-0">
              ...
              >
                ...
                <span>{t('Upload / Manage Documents')}</span>
              </Link>
            </div>
          </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
              ...
              <div className="flex flex-wrap gap-1 mt-0.5">
                <span className="bg-emerald-100 text-emerald-800 text-[9.5px] font-bold px-2 py-0.5 rounded-md">
                  {t('GSTN')}: {isHighRisk ? t('CANCELLED') : t('ACTIVE')}
                </span>
                ...
                >
                  {t('CPPP')}: {isHighRisk ? t('DEBARRED') : t('CLEAR')}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="space-y-1 flex-1">
                <h3 className="text-base font-bold text-amber-900">
                  {t('Official GeM Clarification Notice Issued')}
                </h3>
            ...
            {!clarificationSent ? (
              <form onSubmit={handleSendClarification} className="space-y-3 pt-2">
                <label className="block text-xs font-bold text-amber-900">
                  {t('Submit Bidder Clarification Response')}
                </label>
                <textarea
                  rows={3}
                  value={clarificationResponse}
                  onChange={(e) => setClarificationResponse(e.target.value)}
                  placeholder={t('Enter your formal response, explanation of corporate identity, or cite attached board resolution...')}
                  required
                ...
                >
                  ...
                  <span>{t('Submit Response to Tender Authority')}</span>
                </button>
              </form>
            ) : (
              <div className="p-3 bg-emerald-100 border border-emerald-300 rounded-xl text-xs text-emerald-900 font-bold flex items-center space-x-2">
                ...
                <span>
                  {t('Clarification response submitted successfully! Timestamp recorded in immutable audit log.')}
                </span>
              </div>
            )}
          </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10.5px] font-bold text-slate-500">
                  {bidder.documents?.length || 5} {t('Documents')}
                </span>
              </div>
                    ...
                    <div className="flex items-center space-x-2.5 min-w-0 flex-1 pr-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-slate-400">
                          {doc.doc_type} • {doc.page_count} {t('Pages')}
                        </p>
                      </div>
                    </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-xs space-y-4">
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-bold text-blue-900">{t('R001 • Turnover')}</span>
                    {isHighRisk ? (
                  ...
                  <p className="text-slate-600 text-[11.5px]">
                    {t('Threshold: Minimum ₹3.00 Cr avg annual turnover.')}
                  </p>
                  <p className="text-slate-900 font-semibold text-[11px]">
                    {t('Extracted from CA Audit')}: {isHighRisk ? '₹1.80 Crores' : isMediumRisk ? '₹3.15 Crores' : '₹5.20 Crores'}
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-bold text-blue-900">{t('R002 • Past Experience')}</span>
                    <span className="bg-emerald-100 text-emerald-800 text-[10px] font-bold px-2 py-0.5 rounded-full">
                      {t('COMPLIANT')}
                    </span>
                  </div>
                  <p className="text-slate-600 text-[11.5px]">
                    {t('Threshold: Minimum 5 years operational experience.')}
                  </p>
                  <p className="text-slate-900 font-semibold text-[11px]">
                    {t('Extracted from Completion Certificate: 7 Years continuous track record.')}
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-bold text-blue-900">{t('R003 • GST Registration')}</span>
                    {isHighRisk ? (
                      <span className="bg-red-100 text-red-800 text-[10px] font-bold px-2 py-0.5 rounded-full">
                        {t('CANCELLED / FAILED')}
                      </span>
                  ...
                  <p className="text-slate-600 text-[11.5px]">
                    {t('Live GSTN Registry check verified Form REG-06 status.')}
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-bold text-blue-900">{t('R004 • Non-Debarment')}</span>
                    {isHighRisk ? (
                      <span className="bg-red-100 text-red-800 text-[10px] font-bold px-2 py-0.5 rounded-full">
                        {t('DEBARRED ON CPPP')}
                      </span>
                      ...
                      <span className="bg-emerald-100 text-emerald-800 text-[10px] font-bold px-2 py-0.5 rounded-full">
                        {t('CLEAR / ZERO DEBARMENT')}
                      </span>
                    )}
                  </div>
                  <p className="text-slate-600 text-[11.5px]">
                    {t('Cross-checked against CPPP Central Debarment Database under GFR Rule 151.')}
                  </p>
                </div>
                ...
                <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-mono font-bold text-blue-900">{t('R005 • OEM Authorization (MAF)')}</span>
                    {isMediumRisk ? (
                  ...
                  <p className="text-slate-600 text-[11.5px]">
                    {t('Manufacturer Authorization Form validity and legal entity consistency.')}
                  </p>
                </div>
              </div>
            </div>
