    ...
    <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4 flex flex-col h-full">
        ...
        <div>
          <div className="flex items-center space-x-1.5">
            ...
            <h3 className="text-sm font-bold text-slate-900">
              {t('verify.officer_action', 'Officer Decision & Action Dock')}
            </h3>
          </div>
      ...
      <div className="p-3 bg-slate-50 rounded-lg border border-slate-200 text-xs">
        <span className="text-[10px] uppercase font-bold text-slate-400 block mb-0.5">
          {t('common.status', 'Current Status')}
        </span>
        <span
      ...
      <div className="p-3.5 bg-blue-50/70 rounded-xl border border-blue-200 space-y-1.5 text-xs">
        <div className="flex items-center space-x-1.5 text-blue-900 font-bold">
          ...
          <span>{t('ai.executive_summary', 'Executive Compliance Summary')}</span>
        </div>
      ...
      <div className="space-y-2">
        <label className="text-xs font-bold text-slate-700 block">
          {t('verify.decision_prompt', 'Record Final Adjudication Decision')}:
        </label>
        <div className="grid grid-cols-1 gap-1.5">
          ...
          >
            <span className="flex items-center space-x-2">
              ...
              <span>{t('verify.qualify_btn', 'Approve / Qualify Bidder')}</span>
            </span>
          ...
          >
            <span className="flex items-center space-x-2">
              ...
              <span>{t('verify.clarify_btn', 'Request Clarification')}</span>
            </span>
          ...
          >
            <span className="flex items-center space-x-2">
              ...
              <span>{t('verify.disqualify_btn', 'Disqualify Bidder')}</span>
            </span>
      ...
      <div className="space-y-1.5 flex-1 flex flex-col">
        <div className="flex items-center justify-between">
          <label className="text-xs font-bold text-slate-700 flex items-center space-x-1">
            <span>{t('common.details', 'Officer Remarks')}</span>
            <span className="text-rose-500">*</span>
          </label>
          <span className="text-[10px] text-slate-400 font-mono">{t('nav.audit_trail', 'Audit Log')}</span>
        </div>
        <textarea
          rows={4}
          value={remarks}
          onChange={(e) => setRemarks(e.target.value)}
          placeholder={t('verify.remarks_placeholder', 'Enter official adjudication notes, justification, or statutory citation...')}
      ...
      <div className="pt-2">
        <button
          ...
          className="w-full py-2.5 px-4 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-400 text-white text-xs font-bold rounded-lg flex items-center justify-center space-x-2 transition-colors shadow-xs cursor-pointer"
        >
          {isSubmitting ? (
            <span>{t('common.loading', 'Recording...')}</span>
          ) : savedSuccess ? (
            <>
              <Check className="w-4 h-4 text-emerald-400" />
              <span>{t('status.approved', 'Adjudication Recorded!')}</span>
            </>
          ) : (
            <>
              <Send className="w-3.5 h-3.5" />
              <span>{t('verify.sign_submit', 'Sign & Submit Decision with Class-3 DSC')}</span>
            </>
          )}
        </button>
        <p className="text-[10px] text-center text-slate-400 mt-1.5">
          {t('role.officer_features_2', 'Class-3 PKI DSC token signed adjudication')} • ABCD
        </p>
      </div>
    </div>
  );
